...
class Speech2Text:
    def get_transcript(self, sound) -> str:
        return requests.get("http://192.168.42.5:8000/request/", data=sound.json()).text.strip()
...
class CzytanieApp:
    ...
    def __init__(self):
        ...
        try:
            ...
            with open("total_scores.json", 'w') as fw:
                jsonobj = json.dumps({"accuracy": 0., "time": 0., "correct": 0., "incorrect": 0., "total": 0.},
                                     indent=4)
                fw.write(jsonobj)
        ...
        self._accuracy_score_label = tk.Label(self._window, text=f"Accuracy score: {self.accuracy_score}",
                                              background='black', foreground='white')
        self._accuracy_score_label.pack()

        self._time_score_label = tk.Label(self._window, text=f"Time score: {self.time_score}", background='black',
                                          foreground='white')
        self._time_score_label.pack()

        self._correct_label = tk.Label(self._window, text=f"Correct: {self.correct}", background='black',
                                       foreground='white')
        self._correct_label.pack()

        self._incorrect_label = tk.Label(self._window, text=f"Incorrect: {self.incorrect}", background='black',
                                         foreground='white')
        self._incorrect_label.pack()

        self._total_questions_label = tk.Label(self._window, text=f"Total questions: {self.total_questions}",
                                               background='black', foreground='white')
    ...
    def check_answer(self, transcript):
        ...
        with open("total_scores.json", 'w') as fw:
            jsonobj = json.dumps({"accuracy": self.accuracy_score, "time": self.time_score, "correct": self.correct,
                                  "incorrect": self.incorrect, "total": self.total_questions}, indent=4)
            fw.write(jsonobj)
